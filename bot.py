...
KIE_GENERATE_PATH = os.getenv("KIE_GENERATE_PATH", "/v1/veo3/generations")  # поменяй, если у тебя другой
...
def run_with_format_kb(aspect: str) -> InlineKeyboardMarkup:
    b16  = f"{'✅ ' if aspect=='16:9' else ''}🎬 16:9"
    b916 = f"{'✅ ' if aspect=='9:16' else ''}📱 9:16"
...
def _submit_kie(payload: dict) -> dict:
    if not (KIE_API_KEY and KIE_BASE_URL and KIE_GENERATE_PATH):
        return {"ok": False, "id": None, "error": "KIE_API_KEY / KIE_BASE_URL не заданы."}
...
async def cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data in ("fmt_16x9", "fmt_9x16"):
        ...
        # если промпт готов — держим клавиатуру «формат + запуск»
        if st["mode"] in ("gen_text", "gen_photo") and st.get("last_prompt"):
            try:
                await q.edit_message_reply_markup(reply_markup=run_with_format_kb(st["aspect"]))
            except:  # noqa
                pass
        else:
            # иначе только подсветим выбор
            try:
                await q.edit_message_reply_markup(reply_markup=format_kb(st["aspect"]))
            except:  # noqa
                pass
        return
    ...
    # назад
    if data == "back_menu":
        st["mode"] = None; st["last_prompt"] = None; st["last_image_url"] = None
    ...
    if data == "mode_gen_photo":
        ...
        await q.edit_message_text(
            "📸 Пришли **фото** с подписью (краткое описание). Если подписи нет — отправь фото, затем текст.",
            reply_markup=format_kb(st["aspect"])
        ); return
    ...
    if data == "mode_chat":
        st["mode"] = "chat"
        await q.edit_message_text("💬 Обычный чат. Пиши сообщения. /exit — выход.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад в меню", callback_data="back_menu")]]))
        return
    ...
    if data == "mode_gen_text_from_pm":
        ...
        await q.edit_message_text("Режим «по тексту». Измени формат ниже или жми «🚀».",
                                  reply_markup=run_with_format_kb(st["aspect"]))
        return
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if st["mode"] in (None, "gen_text", "gen_photo"):
        # если в режиме «по фото» ещё нет фото
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        cb,
        pattern=r"^(mode_.+|fmt_16x9|fmt_9x16|run|back_menu|faq)$"
    ))
